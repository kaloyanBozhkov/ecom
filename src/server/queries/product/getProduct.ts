...
const fetchProduct = cache(async (slug: string) => {
  ...
  return {
    ...
    images: product.images as unknown as Product['images'],
    features: product.features as unknown as Product['features'],
    specifications: product.specifications as unknown as Product['specifications'],
...
const fetchAllProducts = cache(async () => {
  ...
  return products.map((p: typeof products[number]): Product => ({
    ...
    currency: p.currency,
    images: p.images as unknown as Product['images'],
    features: p.features as unknown as Product['features'],
